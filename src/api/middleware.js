...
export function allow(auth: IAuth) {
  return function(action: string) {
    return function(req: $RequestExtend, res: $ResponseExtend, next: $NextFunctionVer) {
      ...
      const packageVersion = req.params.filename ? getVersionFromTarball(req.params.filename) : 
        req.body && req.body.versions ? Object.keys(req.body.versions)[0] : undefined;
